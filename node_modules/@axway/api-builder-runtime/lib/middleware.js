...
function Middleware(app, dirname, sessionConfig, config) {
	...
	app.use(function (req, resp, next) {
		if (req.body && _.isObject(req.body) && !(req.body instanceof Buffer)) {
			...
			req.params = req._params = merge(req.params, req.body);
		} else {
	...
	enableStaticServerAndRenderes(dirname, app, config);
...
function objectCustomizer(value) {
	...
	if (value instanceof APIBuilder.Instance || value instanceof APIBuilder.Collection || _.isFunction(value.toJSON)) {
		return value.toJSON();
...
function setupSession(app, sessionConfig) {
	app.use(
		sessions({
			...
			encryptionKey: Buffer.from(sessionConfig.encryptionKey || '8yOAsrmwQ7jcUJC7SS1bBmGLRYW0g0ucOwSDjmSOnKg=', 'base64'),
			signatureAlgorithm: sessionConfig.signatureAlgorithm || 'sha512-drop256',
			signatureKey: Buffer.from(sessionConfig.signatureKey || 'ub9VhdXWtQ6oN2UwhvkiRO0eS9S6OgAbHOAAPjKbW+Qd/A2OYGiZdjcrNliQU+ho4e5Dz5Wj4z2ggfefRjU3ug==', 'base64'),
			cookieName: 'APIBuilder', // cookie name dictates the key name added to the request object - do not changed this or you will get an error
 ...
 * @param {APIBuilderRendererEngine} engine Renderer engine.
 */
 ...
 * @returns {APIBuilderRendererEngine}
 */
/**
 * @method getRendererEngine
...
/**
 * Functionality that enables static server and renderers.
 * Static server is now started out of the middleware file (see comments bellow).
 * For backward compatibility it is still possible to be strted from here if it has not been started from outside (see enableStaticPublicDirectory flag).
 *
 * Renderers are now not registered by default and controlled by enableRenderers flag in the implementation of this function.
 *
 * @param {*} dirname dirname
 * @param {*} app express app
 * @param {*} config app configuration
 */
function enableStaticServerAndRenderes(dirname, app, config) {
	const webDirectory = dirname && path.join(dirname, 'web');
	if (webDirectory && fs.existsSync(webDirectory)) {
		const publicDirectory = path.join(webDirectory, 'public');
		// Static server is now started out of the middleware file.
		// It has been moved out of the Middleware and moved _before_ the authentication middleware.
		// Otherwise, the authentication middleware will now block access by default.
		// To preserve backward compatibility we have flag enableStaticPublicDirectory
		// which might be true to trigger static server within Middleware.
		// The tests that start the project only as a web service with renderer (why I have no idea) are in middleware_test.js - "should create instance that serves static HTML pages".
		if (fs.existsSync(publicDirectory) && config.enableStaticPublicDirectory !== false) {
			app.use(express.static(publicDirectory));
		}
		// enableRenderers === undefined => Middleware is invoked outside of API Builder => always register renders
		// enableRenderers === true/false => Middleware is invoked within API Builder => register renderers when true
		if (config.enableRenderers !== false) {
