...
function Response(req, resp, api) {
	...
	req.session = resp.session = req.APIBuilder;
	req.session && (req.session.destroy = resp.session.destroy = req.APIBuilder.reset);
	...
	resp.format = function format(body) {
	...
	this.flushBody = function () {
		if (alreadySent || self.insideSend) {
			return;
		}
		alreadySent = true;
		...
		// FYI: logging for all requests has been moved to apibuilder middleware
		// functions, createRequestTransaction and endRequestTransaction.
	};
...
Response.prototype.ok
	= Response.prototype.success = function success(object, properties, callback) {
		...
		var response = merge({ success: true, 'request-id': this.request.getId() }, properties || {}),
...
Response.prototype.failure = function failure(status, message, code, url, callback) {
	...
	this.response.logger.trace(message);
...
Response.prototype.withStatus = function withStatus(callback, status) {
	this.response.logger.info({ ignore: true, nocontent: true, status, headers: this.response.headers });
