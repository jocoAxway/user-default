...
function API(impl, config, apibuilder) {
	...
	this.config = merge(impl && impl.config || {}, apibuilder.config, config);
	...
	if (this.models && Array.isArray(this.models)) {
		...
		this.models = this.models.map(function (name) {
			return this.apibuilder.getModel(name, true);
		}.bind(this));
	}
	...
	apibuilder.logger.debug('registering' + (this.enabled ? '' : ' disabled') + ' api', this.method, this.path);
	...
	if (this.eventTransformer) {
		if (typeof this.eventTransformer === 'string') {
 ...
 * Pass the constructor an APIBuilder configuration object, APIBuilder instance, and optionally a filename.
 * @static
...
API.prototype.extend = function instanceExtend(impl) {
	return API.extend(merge(this, impl));
};
				...
				switch (method) {
					...
					case 'POST':
						if (results && _.isFunction(results.getMeta) && results.getMeta('includeResponseBody')) {
							...
							} else if (apiBuilderConfig.flags.enableModelsWithNoPrimaryKey && results.getPrimaryKey() === undefined) {
...
API.prototype.getMiddleware = function getMiddleware() {
	...
	return function MiddlewareConstructor(req, resp, r, next) {
		...
		try {
			...
			req.params = merge(merge({}, req._params), req.params);
...
API.prototype.bind = function (app) {
	...
	Object.keys(formatters.extensions).forEach(function (extension) {
		...
		app[method](api.path + '.' + extension, createAPIBinding(function (req) {
...
API.prototype.execute = function (params, callback) {
	...
	request.logger = response.logger = request.log = response.log = this.apibuilder.logger.scope(request);
	...
	try {
		...
		this.middleware(request, response, r, function (err, result) {
			...
			if (value && (value instanceof APIBuilder.Instance || value instanceof APIBuilder.Collection || _.isFunction(value.toJSON))) {
