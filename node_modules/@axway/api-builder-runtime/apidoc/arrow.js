 ...
 * when creating an API Builder instance, or programmatically create components after initializing an API Builder instance.
 *
 ...
 *
 * API Builder uses Express for its web framework. By default, API Builder creates an Express app instance
 * when creating an API Builder instance, then binds its API endpoints to the app instance.
 ...
 * #### Blocks
 *
 * {@link APIBuilder.Block Blocks} are functions that run before or after an API endpoint is executed.
 ...
 *
 * {@link APIBuilder.Router Routes} provide the endpoint to access the Web and logic to access the data.
 *
 * {@link APIBuilderRendererEngine Renderers} (or template engines) apply local data to your templates
 * to generate HTML. To access a renderer or create your own renderer, use the {@link APIBuilder.Middleware} API.
 *
 ...
 * #### Logger
 *
 * The {@link APIBuilder.Logger} provides a wrapper for [bunyan](https://www.npmjs.com/package/bunyan),
 * a JSON logging utility.  By default, API Builder creates a logger instance, which can be accessed
 * using the `logger` property of the API Builder instance.
 *
 ...
 *
 * At this point, you may add event listeners or programmatically add any other API Builder components.
 *
 * #### Startup Sequence
 *
 ...
 * #### Request Sequence
 *
 * When a client makes a request to the API Builder service, API Builder first authenticates the request,
 ...
 *
 * 1. Authenticate the request. If you are using a custom security plugin, the `validateRequest()` method
 *    is defined.
 ...
 *
 * Each API Builder middleware call (`action` or `execute` property) is passed the same three objects:
 *
 ...
 * @param {Boolean} [loadOnly] Set to `true` to only load the API Builder components.
