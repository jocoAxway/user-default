...
class Swagger {
	 ...
	 * This is to avoid the possibility of the document having globals incorrectly relevant to the incoming
	 * swagger's methods.
	 * @private
		...
		SWAGGER_AND_METHOD_PROPS.forEach(prop => {
			if (swagger[prop] !== undefined && !mergeBlacklist.includes(prop)) {
				Object.keys(swagger.paths).forEach(part => {
					...
					Object.keys(swagger.paths[part]).forEach(verb => {
						// check that we support the verb, and is not path-level parameters, not an extension,
						// and not a ref.
						if (this[verb] && verb !== 'parameters' && verb !== '$ref' && !EXTENSION_PATTERN.test(verb)) {
			 ...
			 */
				...
				.forEach(verb => {
					...
					if (extensionPattern.test(verb) || verb === 'parameters' || verb === '$ref') {
						...
						// clone properties
						SWAGGER_METHOD_PROPS.filter(key => !mergeBlacklist.includes(key)).forEach(key => {
