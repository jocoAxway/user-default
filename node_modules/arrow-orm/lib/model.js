	...
	if (this._promise) {
		...
		for (var name in this) {
			...
			if (typeof fn === 'function') {
				...
				if (this.connector && dispatchers[name] && typeof this[dispatchers[name]] !== 'function') {
					continue;
				}
...
function extendOrReduce(instance, name, definition, extend) {
	...
	model.metadata = merge(_.cloneDeep(instance.metadata), model.metadata);
	model.mappings = merge(_.cloneDeep(instance.mappings), model.mappings);
	if (model.fields) {
		...
		if (extend) {
			...
			Object.keys(model.fields).forEach(function (name) {
				if (name in fields) {
					model.fields[name] = merge(_.cloneDeep(fields[name]), model.fields[name]);
				} else {
	...
	model.methods = merge(_.cloneDeep(instance.methods), model.methods);
...
Model.prototype.createAPI = function createAPI() {
	...
	// If primary key is not auto-generated primary key then allow insert (if it's not set then default to autogen true).
...
Model.prototype.updateAPI = function updateAPI() {
	...
	return {
		...
		action: function updateAction(req, resp, next) {
			req.model.fetch(req.params[pkName], resp.createCallback(null, function putSuccessCallback(model, cb) {
				try {
		...
		if (!(instance instanceof Instance) || instance.isUnsaved()) {
			if (!(instance instanceof Instance)) {
				...
				if (instance.hasOwnProperty(pkName)) {
					return this.findByID(instance[pkName], function findByIDCallback(err, _instance) {
						if (err) {
							return callback(err);
						}
						if (!_instance) {
							return callback('trying to update, couldn\'t find record with primary key: ' + instance.id + ' for ' + this.name);
						}
	...
	= Model.prototype.remove = function remove(instance, callback) {
		...
		if (typeof instance !== 'object') {
			return this.findByID(instance, function findByIDCallback(err, _instance) {
				...
				if (!_instance) {
					return callback('trying to remove, couldn\'t find record with primary key: ' + instance + ' for ' + this.name);
				}
...
Model.prototype.findAndModifyAPI = function findAndModifyAPI() {
	...
	return {
		...
		description: this.findAndModifyDescription || `Modifies a single ${this.singular}. Although the query may match multiple ${this.plural}, only the first one will be modified.`,
		parameters: parameters,
		responses: {
			204: {
...
Model.prototype.hasPrimaryKey = function hasPrimaryKey() {
	return apiBuilderConfig.flags.enableModelsWithNoPrimaryKey ? !(this.metadata && this.metadata.primarykey === null) : true;
};
...
function mergeFields(definedFields, inheritedFields) {
	var retVal = merge({}, inheritedFields, definedFields);
...
function prepareQueryOptions(ctx, options) {
	...
	var validOptions = { where: 1, sel: 1, unsel: 1, order: 1, skip: 1, limit: 1 };
	...
	if (!_.some(validOptions, function (val, key) {
	...
	if (ctx.defaultQueryOptions) {
		options = merge(ctx.defaultQueryOptions, options);
	}
