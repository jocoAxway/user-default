...
function Collection(model, instances) {
	...
	const array = [];
	const isNew = this instanceof Collection;
	const proto = isNew ? getPrototypeOf(this) : Collection.prototype;
	const self = setPrototypeOf(array, proto);
 ...
 * @returns {Array<APIBuilder.Instance>} A copy of the collection.
 */
 ...
 * @param {Number} index The index in the collection to retrieve.
 ...
 * @param {Array<APIBuilder.Instance>/APIBuilder.Instance} instances The model instance(s) to add.
 * @returns {APIBuilder.Collection}
 * @throws {APIBuilder.ORMError} Passed object or array element was not a Model instance
...
function objectCustomizer(obj) {
	if (obj instanceof Instance) {
		...
		return _.cloneDeepWith(obj, objectCustomizer);
	}
	return obj;
}
...
Collection.prototype.toJSON = function toJSON() {
	return _.cloneDeepWith(this, objectCustomizer);
};

/**
 * @method concat
 * Returns a collection that is joined with all the passed collections.
 * @param {...APIBuilder.Collection} Collections Collection(s) to join.
 * @returns {APIBuilder.Collection}
 */
 ...
 * @param {...APIBuilder.Instance} [items] Model instances to insert.
 * @returns {APIBuilder.Collection}
 */
...
[ 'concat', 'reverse', 'slice', 'splice', 'sort', 'filter', 'map' ].forEach(function (name) {
	const _Array_func = this[name];
	Collection.prototype[name] = function () {
		const result = _Array_func.apply(this, arguments);
