		...
		case 'object':
		{
			...
			if (typeof value === 'string') {
				try {
					...
					if (Object.prototype.toString.call(value).toLowerCase() !== `[object ${fieldType}]`) {
						return false;
					}
					this.set(name, value);
					return true;
...
Instance.prototype.validateField = function validateField(name, v) {
	...
	const fieldType = field.type.toLowerCase();
	if (hasValue && Object.prototype.toString.call(value).toLowerCase() !== `[object ${fieldType}]`) {
...
Instance.prototype.validateFields = function validateFields() {
	...
	if (errors.length > 0) {
		if (errors.length === 1) {
			...
			throw new ValidationError(errors.map(e => e.field).join(', '), errors.map(e => e.message).join('\n'));
		}
	}
};
