const { validateJsonPathSelectors, validateJsonPathSelector } = require('./jsonpath');
...
class FlowValidator {
	...
	* @param {object} credentialManager - a credentialManager
	* @constructor
	*/
	constructor(flowGraph, credentialManager) {
	...
	_strongconnect(vertex, stack, stronglyConnectedGroups) {
		...
		this.flowGraph.getChildren(vertex.id, true).forEach(childId => {
			const child = this.flowGraph.getVertex(childId);
			if (child.info.index === undefined) {
				this._strongconnect(child, stack, stronglyConnectedGroups);
	...
	_parseParamValue(param) {
		...
		if (!param.type || param.type === 'jsonpath') {
			if (param.value === undefined) {
				...
				const failures = validateJsonPathSelector(param.value);
				if (failures) {
			...
			// Check that the credential is registered
			const credentials = this.credentialManager && this.credentialManager.getCredentialConfigs
				&& this.credentialManager.getCredentialConfigs() || {};
			if (!credentials[param.value]) {
			...
			} else if (param.type === 'object' && (val === undefined || typeof val !== 'object' || Array.isArray(val) || val === null)) {
	...
	checkForLoops(validationData) {
		...
		this.flowGraph.getVertices().forEach(vertex => {
			if (vertex.info.index === undefined) {
				this._strongconnect(vertex, stack, stronglyConnected);
			}
		});

		const loops = stronglyConnected.filter(
			g => {
				const firstChildId = this.flowGraph.getChildren(g[0].id, true)[0];
				const firstChild = firstChildId && this.flowGraph.getVertex(firstChildId);
	...
	checkForMissingRoutes(validationData) {
		...
		this.flowGraph.getNodes(true).forEach(nodeId => {
			const outputs = this.flowGraph._getOutputRoutes(nodeId);
			Object.keys(outputs).forEach(output => {
				const routes = outputs[output];
				routes.forEach((routeId) => {
					if (!this.flowGraph.getNode(routeId)) {
						invalid = true;
						this._addValidationError(validationData, {
	...
	checkForInvalidStart(validationData) {
		const startNodeId = this.flowGraph.flow.start;
	...
	checkNodeOutputContext(validationData) {
		...
		this.flowGraph.getNodes(true).forEach(nodeId => {
			...
			Object.keys(node.outputs).forEach((output) => {
				const outputContext = node.outputs[output].context;
				if (outputContext) {
					const err = validateJsonPathSelector(outputContext);
					if (err) {
						invalid = true;
						this._addValidationError(validationData, {
	 ...
	 */
	checkReturns(validationData) {
		const returns = this.flowGraph.flow.returns;
		if (returns === undefined || returns === null) {
			return false;
		}

		const errs = validateJsonPathSelectors(returns);
		if (errs) {
			errs.forEach(err => {
				this._addValidationError(validationData, {
					type: 'invalidReturns',
